...
import { ChildrenProps } from "src/packages/utils/react";

import { PlayFim } from "src/packages/components/AudioTags";
import {
  AvailableElections,
...
import { electionsApi } from "src/packages/repository/api";
import {
  addVote,
  getConfiguration,
  getVotes,
...
function VotingCtxProvider({ children }: ChildrenProps) {
  ...
  const mapToNotion = useCallback(
    (voteCounts: { [key: string]: number }): ResultToNotion => {
      const [[winnerCode, winnerVotes], [loserCode, loserVotes]] =
      ...
      const results: ResultToNotion = {
        winner: {
          ...
          name: winnerName,
          votes: String(winnerVotes),
        },
        loser: {
          members: loserMembers,
          name: loserName,
          votes: String(loserVotes),
        },
      };
  ...
  function countingVotes(votes: { code: string }[]) {
    const votedSet = new Set();
    const voteCounts: { [key: string]: number } = {};

    votes.forEach((vote: { code: string }) => {
